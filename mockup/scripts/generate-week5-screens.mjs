...
const screensDir = path.resolve(scriptDir, "../screens/week5");
...
function cleanWeek5HtmlFiles() {
  if (!fs.existsSync(screensDir)) {
    return;
  }
  fs.readdirSync(screensDir, { withFileTypes: true }).forEach((entry) => {
    if (entry.isFile() && entry.name.endsWith(".html")) {
      fs.unlinkSync(path.join(screensDir, entry.name));
    }
  });
}
...
function generate() {
  ...
  write("15_jp_deep_compare_1.html", renderDeepCompare(0, DEEP_COMPARE_PAIR, "jp", DEEP_ITEMS_JP));

  write("15_jp_to_en_start.html", renderEnStartBreak());
  ...
  write("19_en_deep_compare_1.html", renderDeepCompare(0, DEEP_COMPARE_PAIR, "en", DEEP_ITEMS_EN));
