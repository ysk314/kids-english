...
state.slideIndex = clamp(state.slideIndex, 0, WEEK1_SLIDES.length - 1);
...
function buildHintList() {
  ...
  WEEK1_SLIDES.forEach((slide, index) => {
...
function currentSlide() {
  return WEEK1_SLIDES[state.slideIndex] || WEEK1_SLIDES[0];
}
...
function updateSlideActionButtons(slide) {
  ...
  if (elements.subPrevButton && elements.subNextButton) {
    ...
    elements.subNextButton.disabled = !hasSubStep && state.slideIndex >= WEEK1_SLIDES.length - 1;
    if (slide.kind === "end") {
...
function render() {
  ...
  elements.slideCounter.textContent = `${state.slideIndex + 1} / ${WEEK1_SLIDES.length}`;
...
function setState(patch, { broadcast = true } = {}) {
  state = normalizeTimerState({
    ...state,
    ...patch,
    slideIndex: clamp(patch.slideIndex ?? state.slideIndex, 0, WEEK1_SLIDES.length - 1),
    updatedAt: nowMs()
  });
...
async function submitWorkChoice(choiceIndex) {
  ...
  if (!Number.isInteger(choiceIndex) || choiceIndex < 0 || choiceIndex >= slide.choices.length) {
    return;
  }

  await ensureAudioReady();
  const isCorrectChoice = choiceIndex === slide.correctIndex;
  const onMiddleBeat = isWorkTapOnMiddleBeat(slide);
  const isCorrect = isCorrectChoice && onMiddleBeat;
  if (isCorrect) {
    audio.playCorrect();
  } else {
    audio.playIncorrect();
  ...
  setState({
    slideInteractions: setSlideInteraction(slide.id, {
      ...
      answeredAt: nowMs(),
      onMiddleBeat,
      beatStep16: latestBeatStep16,
      judgedResult: isCorrect ? "correct" : "incorrect"
    }),
    fxEvent: makeFxEvent(isCorrect ? "correct" : "incorrect")
  });
}

async function submitDeepStepChoice(choiceIndex) {
  const slide = currentSlide();
  if (slide.kind !== "deep_step" || !Array.isArray(slide.correctIndices) || slide.correctIndices.length < 1) {
    return;
  }
  ...
  await ensureAudioReady();
  const isCorrect = slide.correctIndices.includes(choiceIndex);
  if (isCorrect) {
    audio.playCorrect();
  } else {
    audio.playIncorrect();
...
function setupRealtimeSync() {
  ...
  bus.onSignal((signal) => {
    if (!signal || (signal.name !== "work-choice" && signal.name !== "deep-step-choice")) {
      return;
    }
    ...
    if (signal.name === "work-choice") {
      ...
      submitWorkChoice(choiceIndex);
      return;
    }

    if (slide.kind !== "deep_step" || payload.slideId !== slide.id) {
      return;
    }
